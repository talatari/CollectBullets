...
namespace Source.Codebase.Upgrades
{
    ...
    public class UpgradeHandler : IDisposable
    {
        ...
        // TODO: ApplyUpgrade должен быть по отношению к статс, а не по отношению к апгрейдмодел
        private void ApplyUpgrade(UpgradeModel upgradeModel)
        {
            ...
            switch (upgradeModel.StatType)
            {
                case StatType.Damage:
                    _stats.DamageStats.AddDamage(upgradeModel.IncrementValue);
                    break;

                case StatType.Burning:
                    _stats.DamageStats.AddBurning(upgradeModel.IncrementValue);
                    break;

                case StatType.Vampirism:
                    _stats.DamageStats.AddVampirism(upgradeModel.IncrementValue);
                    break;

                case StatType.ClipCapacity:
                    _stats.DamageStats.AddClipCapacity(upgradeModel.IncrementValue);
                    break;

                case StatType.ShootingDelay:
                    _stats.DamageStats.AddShootingDelay(upgradeModel.IncrementValue);
                    break;

                case StatType.MaxHealth:
                    _stats.HealthStats.AddMaxHealth(upgradeModel.IncrementValue);
                    break;

                case StatType.Regeneration:
                    _stats.HealthStats.AddRegeneration(upgradeModel.IncrementValue);
                    break;

                case StatType.Magnet:
                    _stats.CommonStats.AddMagnet(upgradeModel.IncrementValue);
                    break;

                case StatType.Speed:
                    _stats.CommonStats.AddSpeed(upgradeModel.IncrementValue);
                    break;

                case StatType.Freeze:
                    _stats.CommonStats.AddFreeze(upgradeModel.IncrementValue);
                    break;
                
                case StatType.RadiusAttack:
                    _stats.CommonStats.AddRadiusAttack(upgradeModel.IncrementValue);
                    break;
            }
        }
        //
